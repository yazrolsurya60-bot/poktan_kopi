...
class Laporan_model extends CI_Model
{
	...
	public function get_laporan_petani()
	{
		...
		usort($petani, function ($a, $b) {
			return $b['total_panen'] - $a['total_panen'];
		});
		return $petani;
	}
	...
	{
		// 🔴 PERBAIKI: Hapus 'l.jenis_kopi' karena kolom tidak ada
		$this->db->select('p.*, pt.nama_petani, l.nama_lahan as lahan')
			->from('tb_panen p')
			->join('tb_petani pt', 'pt.id_petani = p.id_user', 'left')
			->join('tb_lahan l', 'l.id_lahan = p.id_lahan', 'left')
			->order_by('p.tanggal_panen', 'DESC');

		$panen = $this->db->get()->result_array();

		if (empty($panen)) return [];

		foreach ($panen as &$pn) {
			// 🔴 Gunakan nilai default jika kolom tidak ada
			$pn['jenis_kopi'] = $pn['jenis_kopi'] ?? 'Liberika';
