...
class Transaksi extends CI_Controller {
    ...
    public function index() {
        ...
        if (empty($id_produk_saya)) {
            ...
            // Ambil transaksi yang mengandung produk petani ini
            // Hanya tampilkan transaksi dari pembeli yang punya akun (bukan guest/tamu)
            $this->db->select('t.*, u.nama as nama_pembeli');
            $this->db->from('tb_transaksi t');
            $this->db->join('tb_user u', 't.id_user = u.id_user', 'inner'); // inner join = transaksi guest (id_user NULL) otomatis kebuang
    ...
    public function detail($id_transaksi) {
        ...
        // DAN bukan transaksi guest (id_user harus ada)
        $this->db->select('d.*');
