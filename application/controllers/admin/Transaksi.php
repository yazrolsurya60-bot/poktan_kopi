...
class Transaksi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['title']        = 'Manajemen Transaksi - Sistem Supply Chain Kopi';
        $data['title_page']   = 'Manajemen Transaksi';
        $data['subtitle']     = 'Kelola data transaksi & pesanan komoditas kopi Anda';
    ...
    public function detail($id_transaksi)
    {
        ...
        $data['subtitle']     = 'Informasi lengkap rincian transaksi';
        $data['notifikasi']   = $this->Notifikasi_model->get_unread_notif($id_user);
        $data['unread_count'] = $this->Notifikasi_model->count_unread($id_user);
        $data['role']         = 'Admin';
    ...
    public function konfirmasi()
    {
        ...
        $data['title']        = 'Konfirmasi Pembayaran - Admin';
        $data['title_page']   = 'Konfirmasi Pembayaran';
        $data['subtitle']     = 'Verifikasi bukti pembayaran dari pembeli';
        $data['notifikasi']   = $this->Notifikasi_model->get_unread_notif($id_user);
        $data['unread_count'] = $this->Notifikasi_model->count_unread($id_user);
        $data['role']         = 'Admin';
        ...
        if ($status == 'Diverifikasi') {
            ...
            if (!empty($transaksi['id_user'])) {
                $this->Notifikasi_model->save_notifikasi([
                    ...
                    'isi_notifikasi' => 'Pembayaran pesanan #' . $id_transaksi . ' diverifikasi.',
            ...
            if (!empty($transaksi['id_user'])) {
                $this->Notifikasi_model->save_notifikasi([
                    ...
                    'isi_notifikasi' => 'Pembayaran pesanan #' . $id_transaksi . ' ditolak.',
                    'link'           => 'pembeli/transaksi/detail/' . $id_transaksi,
                    'icon'           => 'danger'
                ]);
            }
    ...
    {
        // Invoice dibiarkan load tanpa header/footer karena format cetak
        $data['transaksi'] = $this->Transaksi_model->get_transaksi($id_transaksi);
        if (!$data['transaksi']) {
            show_404();
        }
