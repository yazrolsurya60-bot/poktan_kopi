...
<head>
    ...
    <style>
        ...
        .laporan-tabs .tab-content { padding: 24px; }
        ...
        .rank-badge.gold   { background: var(--amber-cream); color: white; }
        .rank-badge.silver { background: #9CA3AF; color: white; }
        .rank-badge.bronze { background: #B45309; color: white; }
        .rank-badge.other  { background: var(--bg-cream); color: var(--text-secondary); }
        ...
        .keu-summary-card.pengeluaran { border-color: #EF4444; background: #FEF2F2; }
        .keu-summary-card.laba { border-color: var(--amber-cream); background: #FFF7ED; }
        ...
        @media (max-width: 991.98px) {
            ...
            .laporan-tabs .nav-tabs { flex-wrap: nowrap; overflow-x: auto; }
        ...
        .laporan-tabs .nav-tabs::-webkit-scrollbar { width: 3px; height: 3px; }
                    ...
                    <table class="table table-custom mb-0">
                        ...
                        <tbody>
                            <?php if (!empty($petani)): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr><td colspan="7" class="text-center py-4 text-muted">Belum ada data petani</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
                    ...
                    <table class="table table-custom mb-0">
                        ...
                        <tbody>
                            <?php if (!empty($produk)): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr><td colspan="8" class="text-center py-4 text-muted">Belum ada data produk</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
                            ...
                            <?php if (!empty($mitra)): ?>
                                <?php foreach ($mitra as $i => $m): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php $rating = (int)($m['rating'] ?? 0); for ($s = 0; $s < 5; $s++): ?>
    ...
    chartPenjualan = new Chart(ctx.getContext('2d'), {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    backgroundColor: '#2C1808', titleColor: '#E6A15C', bodyColor: '#FAF6F0', cornerRadius: 8, padding: 10,
                    callbacks: { label: function(ctx) { return mode === 'volume' ? ctx.parsed.y + ' kg' : 'Rp ' + ctx.parsed.y.toLocaleString('id-ID'); } }
                }
            },
            scales: {
                y: { beginAtZero: true, grid: { color: 'rgba(74,44,17,0.06)' }, ticks: { font: { size: 10 }, color: '#70655E', callback: function(val) { return mode === 'volume' ? val + ' kg' : 'Rp ' + (val/1000000).toFixed(0) + 'jt'; } } },
                x: { grid: { display: false }, ticks: { font: { size: 10 }, color: '#70655E' } }
            },
...
function initChartProduk() {
    ...
    new Chart(ctx.getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{ data: chartProdukData.data, backgroundColor: ['#E6A15C', '#4A2C11', '#10b981', '#6B7280', '#DBEAFE'], borderColor: '#FFFFFF', borderWidth: 3, hoverOffset: 6 }]
        },
        options: {
            responsive: true, maintainAspectRatio: false, cutout: '65%',
            plugins: {
                legend: { position: 'bottom', labels: { font: { size: 10 }, boxWidth: 12, padding: 8 } },
                tooltip: { backgroundColor: '#2C1808', titleColor: '#E6A15C', bodyColor: '#FAF6F0', cornerRadius: 8, callbacks: { label: function(ctx) { return ctx.label + ': ' + ctx.parsed + ' kg'; } } }
            }
        }
    });
}
...
function initChartKeuangan() {
    ...
    new Chart(ctx.getContext('2d'), {
        ...
        data: {
            ...
            datasets: [
                { label: 'Pendapatan', data: chartKeuanganData.pendapatan, backgroundColor: 'rgba(16, 185, 129, 0.7)', borderColor: '#10b981', borderWidth: 1, borderRadius: 6 },
                { label: 'Biaya', data: chartKeuanganData.pengeluaran, backgroundColor: 'rgba(239, 68, 68, 0.7)', borderColor: '#EF4444', borderWidth: 1, borderRadius: 6 },
                { label: 'Laba', data: chartKeuanganData.laba, backgroundColor: 'rgba(230, 161, 92, 0.85)', borderColor: '#E6A15C', borderWidth: 1, borderRadius: 6, type: 'line', tension: 0.4, fill: false, pointRadius: 4 }
            ]
        },
        options: {
            ...
            plugins: {
                ...
                tooltip: { backgroundColor: '#2C1808', titleColor: '#E6A15C', bodyColor: '#FAF6F0', cornerRadius: 8, callbacks: { label: function(ctx) { return ctx.dataset.label + ': Rp ' + ctx.parsed.y.toLocaleString('id-ID'); } } }
            },
            scales: {
                y: { beginAtZero: true, grid: { color: 'rgba(74,44,17,0.06)' }, ticks: { font: { size: 10 }, color: '#70655E', callback: function(val) { return 'Rp ' + (val/1000000).toFixed(0) + 'jt'; } } },
                x: { grid: { display: false }, ticks: { font: { size: 10 }, color: '#70655E' } }
            }
        }
    });
...
if (notifToggle) notifToggle.addEventListener('click', (e) => { e.stopPropagation(); notifDropdown.classList.toggle('show'); });
document.addEventListener('click', (e) => { if (notifDropdown && !notifDropdown.contains(e.target) && notifToggle && !notifToggle.contains(e.target)) notifDropdown.classList.remove('show'); });
...
if (chartTypeSelect) chartTypeSelect.addEventListener('change', function() { initChartPenjualan(this.value); });
$('#tab-keuangan').on('shown.bs.tab', function() { initChartKeuangan(); });

function updateDateTime() {
    const el = document.getElementById('currentDateTime');
    if (el) el.textContent = new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
}
updateDateTime(); setInterval(updateDateTime, 60000);

document.addEventListener('DOMContentLoaded', function() { initChartPenjualan('pendapatan'); initChartProduk(); });
