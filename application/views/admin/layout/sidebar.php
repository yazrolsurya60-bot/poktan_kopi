<?php
// Ambil data untuk badge sidebar secara dinamis agar selalu muncul di semua halaman
$ci =& get_instance();

$user_baru = $ci->db->where('DATE(created_at)', date('Y-m-d'))->count_all_results('tb_user');
$petani_baru_count = $ci->db->where('role', 'Petani')->where('is_verified', '0')->count_all_results('tb_user');
$transaksi_pending = $ci->db->where('status_pesanan', 'Pending')->count_all_results('tb_transaksi');
$mitra_baru = $ci->db->table_exists('tb_mitra') ? $ci->db->where('status_mitra', 'Pending')->count_all_results('tb_mitra') : 0;
$kurir_baru = 0; // Tidak ada konsep kurir baru yang menunggu approval, admin yang menambahkan kurir.
		...
		<ul class="sidebar-menu">
			...
			<li class="menu-item <?= strpos($current_url, 'admin/kurir') !== false ? 'active' : '' ?>">
				<a href="<?= base_url('admin/kurir'); ?>">
					...
					<?php if ($kurir_baru > 0): ?>
						<span class="menu-badge" style="background: #F59E0B; color: white;"><?= $kurir_baru; ?></span>
					<?php endif; ?>
				</a>
			</li>
