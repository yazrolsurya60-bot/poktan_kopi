    ...
    <script>
        ...
        // ============================================
        // 4. REAL-TIME NOTIFICATION & SOUND EFFECT (ADMIN STYLE)
        // ============================================
        ...
        function fetchRealtimeNotifications() {
            $.ajax({
                url: '<?= base_url("pembeli/dashboard/get_notifications_ajax"); ?>',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if (response.success) {
                        ...
                        // Jika jumlah notifikasi bertambah, bunyikan suara & getarkan lonceng
                        ...
                        if (response.notifikasi && response.notifikasi.length > 0) {
                            ...
                            $.each(response.notifikasi, function(i, n) {
                                ...
                                htmlList += `
                                    <a class="notif-item ${n.status_baca == '0' ? 'unread' : ''}" href="<?= base_url('pembeli/dashboard/read/'); ?>${n.id_notifikasi}">
